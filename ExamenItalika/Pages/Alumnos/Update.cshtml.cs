...
namespace ExamenItalika.Pages.Alumnos
{
	public class UpdateModel : PageModel
	{
		...
		{
			Alumno = GetAlumnoByIdAsync(id).Result;
			Profesores = GetProfesoresAsync().Result;
			return Page();
		}

		private async Task<Models.DTO.Alumno> GetAlumnoByIdAsync(int id)
		{
			var client = _httpClientFactory.CreateClient("MyApi");
			var response = await client.GetAsync("/Alumnos/ById/" + id);
		...
		public async Task<IActionResult> OnPostAsync()
		{
			...
			var client = _httpClientFactory.CreateClient("MyApi");
			var response = await client.PutAsync("/Alumnos/Update", content);
